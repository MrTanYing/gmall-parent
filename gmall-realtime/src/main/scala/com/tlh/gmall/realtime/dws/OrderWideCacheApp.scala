package com.tlh.gmall.realtime.dws

import java.util

import com.alibaba.fastjson.JSON
import com.alibaba.fastjson.serializer.SerializeConfig
import com.tlh.gmall.realtime.bean.{OrderDetail, OrderInfo, OrderWide}
import com.tlh.gmall.realtime.util.{KafkaUtil, OffsetManagerUtil, RedisUtil}
...
object OrderWideCacheApp {
  ...
  def main(args: Array[String]): Unit = {
    ...
    if (orderInfoOffsetMapForKafka != null && orderInfoOffsetMapForKafka.size > 0) { //根据是否能取到偏移量来决定如何加载kafka 流
      orderInfoRecordInputDstream = KafkaUtil.getKafkaStream(orderInfoTopic, ssc, orderInfoOffsetMapForKafka, orderInfoGroupId)
    } else {
      orderInfoRecordInputDstream = KafkaUtil.getKafkaStream(orderInfoTopic, ssc, orderInfoGroupId)
    }
    ...
    if (orderDetailOffsetMapForKafka != null && orderDetailOffsetMapForKafka.size > 0) { //根据是否能取到偏移量来决定如何加载kafka 流
      orderDetailRecordInputDstream = KafkaUtil.getKafkaStream(orderDetailTopic, ssc, orderDetailOffsetMapForKafka, orderDetailGroupId)
    } else {
      orderDetailRecordInputDstream = KafkaUtil.getKafkaStream(orderDetailTopic, ssc, orderDetailGroupId)
    }
